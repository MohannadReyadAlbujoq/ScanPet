...
1. **Items API pagination not working** — `GET /api/items?pageNumber=2&pageSize=5` always returned all items because the handler loaded all items into memory then paginated. Page size and page number were ignored in practice.
...
| Key | Value |
|-----|-------|
| `ConnectionStrings__DefaultConnection` | `Host=ep-soft-recipe-a4u2bjus-pooler.us-east-1.aws.neon.tech;Database=neondb;Username=neondb_owner;Password=npg_9zACCPHxX4VuZ;SSL Mode=Require` |
...
| Git email | `mohannadalbujoq@gmail.com` | `elhamalsalalhi31@gmail.com` |
...
### Session 5 — Inventory-Based Quantity System, Transaction Isolation, New APIs

**Date**: Current session

#### Summary

Removed the deprecated `Quantity` field from the `Item` entity. All quantity tracking now flows exclusively through `ItemInventory` records. Added `PUT /api/inventories/items` for absolute quantity updates, `POST /api/inventories/increment` and `POST /api/inventories/decrement` for relative changes. All inventory-modifying commands and the refund endpoint now use **Serializable** transaction isolation for full ACID compliance.

#### Changes Made

| # | File | Change |
|---|------|--------|
| 1 | `Item.cs` | **Removed** `Quantity` property. Quantity is now solely tracked through `ItemInventories`. |
| 2 | `ItemConfiguration.cs` | **Removed** `Quantity` column config and its index. |
| 3 | `ItemDto.cs` | **Removed** `Quantity` field. |
| 4 | `CreateItemCommand.cs` | **Removed** `Quantity` property. |
| 5 | `CreateItemCommandHandler.cs` | **Removed** `Quantity` from entity creation and audit message. |
| 6 | `UpdateItemCommand.cs` | **Removed** `Quantity` property. |
| 7 | `UpdateItemCommandHandler.cs` | **Removed** `Quantity` from property update and audit. |
| 8 | `CreateItemValidator.cs` | **Removed** `Quantity` validation rule. |
| 9 | `UpdateItemValidator.cs` | **Removed** `Quantity` validation rule. |
| 10 | `ItemsController.cs` | **Removed** `Quantity` from Create/Update command mapping. |
| 11 | `GetAllItemsQueryHandler.cs` | **Removed** `Quantity` from DTO mapping. |
| 12 | `GetItemByIdQueryHandler.cs` | **Removed** `Quantity` from DTO mapping. |
| 13 | `SearchItemsQueryHandler.cs` | **Removed** `Quantity` from DTO mapping. |
| 14 | `ItemRepository.cs` | **Updated** `GetAvailableItemsAsync`, `IsItemAvailableAsync`, `GetLowStockItemsAsync` to use `ItemInventories` instead of `Item.Quantity`. |
| 15 | `DbSeeder.cs` | **Removed** `Quantity` from seed data items. |
| 16 | `CreateOrderCommandHandler.cs` | **Rewritten** — Now checks `GetTotalQuantityForItemAsync()` for stock availability and deducts from `ItemInventory` records (FIFO strategy) instead of `Item.Quantity`. |
| 17 | `CancelOrderCommandHandler.cs` | **Updated** — Restores quantities to `ItemInventory` records instead of `Item.Quantity`. |
| 18 | `IUnitOfWork.cs` | **Added** `BeginTransactionAsync(IsolationLevel)` overload. |
| 19 | `UnitOfWork.cs` | **Added** `BeginTransactionAsync(IsolationLevel)` implementation. |
| 20 | `TransactionBehavior.cs` | **Updated** — Uses **Serializable** isolation for inventory-critical commands (Inventory*, Refund*, CreateOrder*, CancelOrder*). Uses ReadCommitted for other commands. |
| 21 | `UpdateItemQuantityCommand.cs` | **Created** — New command for absolute quantity update at a specific inventory. |
| 22 | `UpdateItemQuantityCommandHandler.cs` | **Created** — Handles `PUT /api/inventories/items` with Serializable transaction. |
| 23 | `IncrementInventoryCommand.cs` | **Created** — New command to increment quantity. |
| 24 | `IncrementInventoryCommandHandler.cs` | **Created** — Adds quantity, warns if exceeds max capacity. |
| 25 | `DecrementInventoryCommand.cs` | **Created** — New command to decrement quantity. |
| 26 | `DecrementInventoryCommandHandler.cs` | **Created** — Subtracts quantity, errors if below 0, warns if below min. |
| 27 | `UpdateItemQuantityDto.cs` | **Created** — DTO for PUT /api/inventories/items. |
| 28 | `IncrementInventoryDto.cs` | **Created** — DTO for POST /api/inventories/increment. |
| 29 | `DecrementInventoryDto.cs` | **Created** — DTO for POST /api/inventories/decrement. |
| 30 | `InventoriesController.cs` | **Added** `PUT /api/inventories/items`, `POST /api/inventories/increment`, `POST /api/inventories/decrement`. |
| 31 | Migration `RemoveItemQuantityColumn` | **Created** — Removes `Quantity` column from `Items` table. |
| 32 | `ScanPet_API.postman_collection.json` | **Updated** — Removed `quantity` from item create/update, added new inventory endpoints. |
| 33 | `SOLUTION_KNOWLEDGE_BASE.md` | **Updated** — This section. |
| 34 | `CreateOrderCommandHandlerTests.cs` | **Updated** — Tests now use inventory mocks. |
| 35 | `RefundOrderItemCommandHandlerTests.cs` | **Updated** — Tests now mock inventory operations. |
| 36 | `CreateItemCommandHandlerTests.cs` | **Updated** — Removed Quantity assertions. |

#### Transaction Isolation — ACID Compliance

All inventory-modifying operations now use **Serializable** isolation level:

```
TransactionBehavior Pipeline:
1. Detect if command is inventory-critical ? use Serializable
2. Begin transaction with Serializable isolation
3. Execute handler (validates, reads, writes)
4. Commit transaction
5. On error ? rollback

Commands using Serializable isolation:
- SetItemInventoryCommand
...
**4 ACID Properties enforced:**
| Property | How Enforced |
|----------|-------------|
| **Atomicity** | `TransactionBehavior` wraps entire handler — commit or rollback |
| **Consistency** | Handlers validate (no negative stock, item/inventory existence) |
| **Isolation** | `IsolationLevel.Serializable` — prevents dirty reads, non-repeatable reads, phantom reads, lost updates |
| **Durability** | PostgreSQL WAL (Write-Ahead Logging) ensures committed data persists |

#### New API Endpoints

| Method | Endpoint | Purpose |
|--------|----------|---------|
| `PUT` | `/api/inventories/items` | Set absolute quantity (with min/max thresholds) |
| `POST` | `/api/inventories/increment` | Add quantity (warns if > max) |
| `POST` | `/api/inventories/decrement` | Remove quantity (error if < 0, warns if < min) |

#### Decrement Error/Warning Behavior

```
quantity - decrement < 0       ? Error 400: "Cannot decrement by X. Current quantity is Y."
quantity - decrement < minimum ? Warning in response notes: "Quantity below minimum threshold"
quantity - decrement == 0      ? Warning: "Item is now out of stock at this inventory"
```

#### Increment Warning Behavior

```
quantity + increment > maximum ? Warning in response notes: "Quantity exceeds maximum capacity"
